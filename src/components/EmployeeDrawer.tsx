          ...
          <div className="flex-1 p-6 overflow-y-auto">
            ...
            {activeTab === 'is' && (
              <div className="space-y-6">
                ...
                {/* Sistem Erişim Hesabı & Yetki Rolü */}
                <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-bold text-blue-900 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-blue-600" />
                      Sistem Erişim Hesabı & Oturum Bilgileri
                    </h4>
                    <span className="text-[11px] font-semibold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Otomatik Oluşturulur</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-gray-200 text-gray-800 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-500"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Başlangıç Şifresi</label>
                      <input
                        type="text"
                        defaultValue="987654"
                        readOnly
                        className="w-full bg-gray-100 border border-gray-200 text-gray-700 font-mono rounded-xl px-3 py-2 text-sm outline-none cursor-not-allowed"
                        title="Tüm kullanıcılar için varsayılan şifre 987654 ataması yapılır, ilk girişte değiştirebilirler."
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Sistem Yetki Rolü</label>
                      <select
                        value={formData.role || 'employee'}
                        onChange={(e) => handleInputChange('role', e.target.value)}
                        className="w-full bg-white border border-gray-200 text-gray-800 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-500"
                      >
                        ...
                        <option value="hr">İK Uzmanı</option>
                        <option value="manager">Müdür</option>
                        {appRole === 'superadmin' && <option value="admin">Şirket Yöneticisi</option>}
                      </select>
                    </div>
                  </div>
                </div>
