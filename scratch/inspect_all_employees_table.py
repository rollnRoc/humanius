...
all_emp = requests.get(f"{supabase_url}/rest/v1/employees?select=id,name,email,company_id,department,position", headers=headers).json()

print(f"Total rows in employees table: {len(all_emp)}")
for e in all_emp[:15]:
    print(f"  ID: {e['id']} | Name: {e['name']} | Email: {e['email']} | Comp: {e['company_id']} | Pos: {e.get('position')} | Dept: {e.get('department')}")
